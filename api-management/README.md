...
* Access to the free [**WireMock**](https://login.wiremock.cloud/) to simulate backend APIs, or any other tool that can expose a test or mock backend API

## Lab 1

Let's create the a mock backend API in Wiremock.

* Start creating a Mock API in <https://app.wiremock.cloud/mock-apis/create>
  * Choose "Unstructured" option to click on "Create Mock API"
  * Set "mock backend" as API name and click Save
...
In this lab, we will proxify our backend API.

* Create a new project in Amplify Fusion to manage this API. Use a unique name in case your not the only one to do this lab on your Amplify Fusion tenant (e.g. XX_ContactAPI with XX being your name or initials).\
  ![new-project](images/lab2-new-project.png)

* Create a new API (e.g. ContactAPI) by uploading this sample OpenAPI specification (OAS) : **[ContactAPI.yaml](assets\ContactAPI.yaml)**.\
  ![api-create](images/lab2-api-create.png)

* Update the backend Server URL to your mock backedn API created in the previous lab
  ![api-create](images/lab2-api-create2.png)

* Edit the API settings to set a front-end base path . Use a unique name in case your not the only one to do this lab on your Amplify Fusion tenant (e.g. XX_api)
...
* Copy the API base url
  ...
  > Note that the url corresponds to the API server of the selected dataplane, followed by the base path you chose in one of the step above

* Test every method from Postman or curl as follows:  
  > Test the following operations after replacing the `{{api_base_url}}` by the the base url you've just copied:
...
In this lab we are going to see, how we can handle the backend security

* Modify the mock back-end API to add some security
...
* Navigate to the backend HTTP/S client connection from you API
...
* Click update
* Click on the Test button and make sure it a success.
![navigate-backend-connection](images/lab3-http-client-connection3.png)
* Go back to the API and activate it
* Test every method again from Postman or curl as follows, and make sure you now get success responses from back-end

## Lab 4

In this lab we are going to see, how to implement an API operation instead of simply proxifying the backend. The implementation on the "Find Contacts" operation, will consists in
...
3. building customized responses based on backedn response if any.
...
* The integration tab should open by itself, click on API Server component and check that linked API operation is  "GET /contacts"
* Expand Operations component and its Status component to see the integration flow. You can add some labels to them.
  ![integration-init](images/lab4-integration-init.png)
* Click the first `+` button in Operations and add a If-Else component. Expand the If-else and label it "check input query params"
...
* Click on Expression and add conditions to make sure at least one query parameter is provided (i.e. it is not null)
  ...
  > Make sure to select OR operators between each expression (Leaving thr AND operator would make all query parameters mandatory, while just want one at least)
* Add a HTTP/S Client GET component if the expression is true
  ![http-client-component](images/lab4-http-client-component.png)
  * Expand the bottom panel
  * Select the `ContactAPI` backend connection (with wiremocks details)
  ...
  * on the output side (right side) of the pipeline:
    * Link backend output statusCode `HTTPSGetOutput/statusCode` to the ouput  `status` variable
    * Right click anywhere in the right hand panel and select Extract and paste in the following JSON that describes the currency converter API response object and click on Copy Node button
      ...
      * Right click again and select Paste and name your variable `backendResponse`
      * Drag a line from ACTION PROPERTIES `HTTPSGetOutput->response` to the `backendResponse` extract variable
      * Right click again anywhere in the right hand panel and select Extract and paste in the following JSON that describes the currency converter API response object and click on Copy Node button
...
* Add a Map component in the else route
  ...
  * Set values for the following output variables
    ...
    * `find-contactsAPIServerResponse/400/message` = `Missing at least one query parameter for you research`
...
In this lab we are going to see, how we can set front-end security on our API and to manage consumer application access and credentials.

Let's update the API to add front-end security:

* Go back to your API in your Amplify Fusion project in the Designer
* Deactivate the API from the data plane
* Open Security tab in the API configuration
...
* Save the changes of API configuration
* Activate your API on the data plane

Now that the API is secured, let's declare a consumer application and give access to our API and generates an API key.

> **Note**: For this part you will need specific Manager privileges. You need to be able access the Applications in the Manager Module, and to create a new application in DESIGN mode. If you can't do the following steps, please reach out to your environment administator to either get your role updated accordingly if possible, or to have him or her do the following step and give you the key at the end.
...
* Create a new Application
  * Give it a name and a description. Make sure the name is unique if other application are created
  * Select your API to git access to it
  * Select the dataplane wher the API is deployed
  * Click on Add API Key
![application-create](images/lab5-application-create.png)
* Click on Save so that the Key is generated and pushed to the linked Dataplane
* Copy the API Key
![application-copy-key](images/lab5-application-copy-key.png)

Let's use our API Key and test our secure API:

* Test the some of the API operations with the API Key and make sure you have a success:
  > Use the following commands in Postman or curl and replace`{{api_base_url}}` by the the base url of the API and `{{api_key}}` by the Key you just copied:
...
* Change the key value in your tests and make sure you have a security error (403)
